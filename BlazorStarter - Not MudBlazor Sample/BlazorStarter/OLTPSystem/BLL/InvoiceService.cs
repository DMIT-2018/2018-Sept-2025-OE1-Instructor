...
namespace OLTPSystem.BLL
{
    public class InvoiceService
    {
        ...
        public Result<int> DeleteInvoice(int invoiceID)
        {
            ...
            if(invoiceID <= 0)
            {
                result.AddError(new Error("Missing Information", "An invoice ID must be provided."));
                return result;
            }
            ...
            if(invoice == null)
            {
                result.AddError(new Error("Missing Invoice", $"Invoice No {invoiceID} does not exist or has been deleted already."));
                return result;
            }
            ...
            try
            {
                //Remember SaveChanges() returns how many rows were changed
